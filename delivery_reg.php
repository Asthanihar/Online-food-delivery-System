	...
	if(isset($_POST['del_name'])&&isset($_POST['del_email'])&&isset($_POST['del_num'])&&isset($_POST['del_dob'])&&isset($_POST['del_address'])&&isset($_FILES['del_resume'])||isset($_POST['del_sex']))
	{
		...
		if(!empty($del_name)&&!empty($del_email)&&!empty($del_num)&&!empty($del_sex)&&!empty($del_dob)&&!empty($del_address))
		{
			...
			if($run=mysqli_query($conn,$query)&&move_uploaded_file($tmp_name, $location.$file_name))
			{
				...
				$message='Thanks for being interested in joining our team. We will verify your resume and details and will send you a official confirmation from our team. Until then, Have a great day.';
				$headers="From: astha.nihar23@gmail.com";
				if(mail($to, $subject, $message, $headers)){
				echo "<script>wnidow.alert('Successfully registered!! We sent you mail and wait for confirmation')</script>";
				echo "<script>window.location.href='delivery_login.php';</script>";}
				else{
					echo "Email Failed";
				}
			}
...
<form action="delivery_reg.php" method="post" enctype="multipart/form-data">
	<table>
		...
		<tr>
			...
			<td>
				<input type="email" name="del_email" placeholder="Email id">
			</td>
		</tr>
		<tr>
			<td>
